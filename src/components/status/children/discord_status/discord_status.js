...
export default function DiscordStatus({ data }) {
  return (
...
const renderDiscordIcon = () => {
  return (
    <svg
      className="h-10 w-10 mx-2 mt-[-2px] text-[#5865F2] dark:text-white"
      viewBox="0 0 71 55"
      fill="none"
...
const renderGameStatus = (data) => {
  if (!data?.activities[0]?.emoji?.name)
    return (
      <div className="capsize text-gray-500 dark:text-gray-100  max-w-max truncate flex items-center space-x-1">
        {data?.activities[0]?.emoji && (
          <Image
            src={`https://cdn.discordapp.com/emojis/${data?.activities[0]?.emoji?.id}.png`}
            height={20}
            width={20}
